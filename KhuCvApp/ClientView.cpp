...
CClientView::CClientView(wxWindow* parent)  {
    ...
    m_nLastSelImageNum = -1;

    Create(parent, wxID_ANY);// , wxPoint(0, 0), parent->GetClientSize());
...
void CClientView::OnPaint(wxPaintEvent& event) {
    ...
    for (CKcImage& kcImage : ((CMainFrame*)(GetParent()))->m_ImageList) {
...
int CClientView::GetPosImage(const wxPoint &pos) {
    for (int i = ((CMainFrame*)(GetParent()))->m_ImageList.size() - 1; i >= 0; --i) {
        CKcImage& kcImage = ((CMainFrame*)(GetParent()))->m_ImageList[i];
...
void CClientView::OnMouseLeftDown(wxMouseEvent& event) {
    ...
    if(m_nSelRegionIndex < 0) {
        ...
        if(m_nCurrentGrabImageNum >= 0){
            int nLastViewImageNum = ((CMainFrame*)(GetParent()))->m_ImageList.size()-1;

            std::swap(((CMainFrame*)(GetParent()))->m_ImageList[m_nCurrentGrabImageNum],
                ((CMainFrame*)(GetParent()))->m_ImageList[nLastViewImageNum]);
...
void CClientView::OnMouseMotion(wxMouseEvent& event) {
    if(m_nSelRegionIndex < 0) {
        ...
        CKcImage& kcImge = ((CMainFrame*)(GetParent()))->m_ImageList[m_nCurrentGrabImageNum];
...
void CClientView::OnMouseLeftUp(wxMouseEvent& event) {
    ...
    if(m_nSelRegionIndex < 0) {
        ...
        ((CMainFrame*)(GetParent()))->m_ImageList[m_nCurrentGrabImageNum].pos += event.GetPosition() - m_SaveGrabPoint;
...
void CClientView::OnMouseLeftDblClk(wxMouseEvent& event) {
    ...
    if (m_nCurrentGrabImageNum >= 0) {
        CKcImage kcImage;
        kcImage.cvImage = ((CMainFrame*)(GetParent()))->m_ImageList[m_nCurrentGrabImageNum].cvImage.clone();
        kcImage.pos = ((CMainFrame*)(GetParent()))->m_ImageList[m_nCurrentGrabImageNum].pos;

        ((CMainFrame*)(GetParent()))->ClearAllImages();
        ((CMainFrame*)(GetParent()))->m_ImageList.push_back(kcImage);
...
void CClientView::OnCopy(wxCommandEvent& event) {
    if (m_nLastSelImageNum < 0 || ((CMainFrame*)GetParent())->m_ImageList.size() <= m_nLastSelImageNum) return;

    cv::Mat cvImage;
    cvImage = ((CMainFrame*)GetParent())->m_ImageList[m_nLastSelImageNum].cvImage.clone();
...
void CClientView::OnDuplicate(wxCommandEvent& event) {
    ...
    CKcImage kcImage = ((CMainFrame*)GetParent())->m_ImageList[m_nLastSelImageNum];
    cv::Mat cvImage = ((CMainFrame*)GetParent())->m_ImageList[m_nLastSelImageNum].cvImage.clone();
...
void CClientView::OnZoomIn(wxCommandEvent& event) {
    ...
    CKcImage kcImage = ((CMainFrame*)GetParent())->m_ImageList[m_nLastSelImageNum];
    cv::Mat cvImage;
    cv::Size cvSize = ((CMainFrame*)GetParent())->m_ImageList[m_nLastSelImageNum].cvImage.size();

    cv::resize(((CMainFrame*)GetParent())->m_ImageList[m_nLastSelImageNum].cvImage, cvImage,
